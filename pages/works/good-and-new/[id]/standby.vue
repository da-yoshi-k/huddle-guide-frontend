...
const handleWorkshopFinished = async () => {
  if (store.workshop?.workshop.work_step.name === '終了') {
    ...
    await navigateTo(`/works/good-and-new/${store.workshop?.workshop.id}/complete`)
  }
}
...
const fetchWorkshopInfo = async () => {
  await store.fetchWorkshop(route.params.id as string).then(async () => {
    ...
    if (!['待機', '終了'].includes(store.workshop!.workshop.work_step.name)) {
      ...
      navigateTo(`/works/good-and-new/${store.workshop?.workshop.id}`)
    }
  })
}
      ...
      <div id="work-info-container" class="flex flex-col mb-8">
        ...
        <div class="mb-8">
          ...
          <ul class="list-decimal ml-8">
            <li class="text-black">自分のカードの鉛筆ボタンをクリックし、最近あった良かったことや新しく知ったことを入力します。</li>
            ...
            <li class="text-black">終了画面ではワーク内で投稿された内容が表示されるので、会話のきっかけにしましょう。</li>
          </ul>
        </div>
        <div class="mb-8">
